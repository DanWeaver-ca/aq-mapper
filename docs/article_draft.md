...
7. **Smoke day vs clean day** — the same campus PM2.5 map from two runs on the shared health-band scale: run 1 (15 July, wildfire smoke — outdoor median 79 µg/m³, 97% of outdoor readings > 50, N95s issued) beside run 3 (12 Aug, clear — outdoor median 11). Likely the strongest figure in the piece, and probably **replaces** the sample-data version of figure 2. Status 2026-08-12: per-run data organized in `student_measurements_2026/<date>/` (gitignored — never committed) and a **prototype built** (matplotlib two-panel, app health-band colours, circles outdoor / squares indoor). For the final version: pseudonymize group labels first (item 1), cap the colourbar near ~150 so the one 768 µg/m³ outlier doesn't stretch it, add a light basemap + scale bar, and decide whether the outlier point is annotated (it is a good story) or filtered (QC discussion in caption either way).
