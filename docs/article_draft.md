...
Running the activity three times added an axis we had not planned: **between days**. One run [which run — confirm date] coincided with a wildfire-smoke peak over Toronto, with outdoor PM2.5 above 50 µg/m³ — we issued N95 masks (3M 1870+ Aura) for the outdoor legs, a live demonstration that the thresholds colouring the map are the thresholds that govern action. On the absolute health-band scale, the same campus that renders green on a clean day rendered [orange/red — confirm] through the smoke; the indoor/outdoor comparison [inverted, with filtered indoor air cleaner than outdoors — confirm against the data], upending the usual indoor-CO₂-is-the-problem story. Two class maps side by side — smoke day and clean day, one colour scale — connect a continental-scale event to the students' own afternoon in a way no downloaded AQI chart can.
...
7. **Smoke day vs clean day** — the same campus PM2.5 map from two runs on the shared health-band scale: one during the Toronto wildfire-smoke peak (outdoor PM2.5 > 50 µg/m³, N95s issued), one clean. Likely the strongest figure in the piece, and probably **replaces** the sample-data version of figure 2. Dependencies: (a) the real-data decision in item 1 — this comparison *cannot* be faked with sample data, which is itself an argument for deciding yes; (b) locating the retained per-run CSVs for all relevant runs [confirm the run-1/run-2 files still exist and which run was the smoke day]; (c) build as a dedicated two-panel static figure from the run CSVs with one shared colourbar (small script — do not screenshot the dashboard for print).
