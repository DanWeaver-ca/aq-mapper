...
## Next Steps (2026 season COMPLETE — the project is in publication mode; list refreshed 2026-08-12)
1. **Edit the publication draft** — the declared next step. `docs/article_draft.md` is at v0.2 with the full three-run arc, real numbers, and the wildfire comparison. **8 bracket sites remain, all instructor-known facts:** program name + cohort sizes (intro), sensor price (×2), the debrief's actual PM2.5 highlight location, GPS-anecdote confirm, AA-roof-discussion confirm, photo anecdote, acknowledgments. **Budget math:** body ≈ 2,470 words; TPT counts each figure as 200 within its ≤3,000 cap → at 4–5 figures the draft is ~270–470 over. Trim from "Why air quality" ¶1 and the standfirst — never from the three-run arc. Claim disciplines + venue plan live in the draft's venue notes.
2. **Figures**: fig 7 (smoke vs clear) draft is BUILT — `docs/figures/make_fig_smoke_vs_clean.py`, committed; data stays in gitignored `student_measurements_2026/`. Pending polish choices: neutral legend markers, the half-clipped western point, optional landmark labels. Figs 1–6: regenerate from sample data. **Real-dataset Zenodo release decision**: NOTES verified clean; gate is a ~10-min group-label pseudonymization pass (runs 1–2 labels include real student names).
3. **Polish round ≈ JOSE checklist** (`docs/v2_improvement_plan.md` §5): CI running analyze+test+hub smoke with badges, CITATION.cff, Zenodo DOI at submission (both papers cite the frozen version), README screenshots/GIF from sample data; then draft JOSE `paper.md` (carry over the secured-by-lifetime endpoint lifecycle and the team-name privacy briefing).
4. **Housekeeping, small**: delete the class Google sheet (target ≈2026-08-19; deployment already archived; local copies verified by analysis — but also back up `student_measurements_2026/` to OneDrive, it currently exists only on the Mac); fold "pick a fun team name, not your names" into `make_handout.py` wording; retire or fix the stale `AQ_Mapper_Student_Handout.docx`; next-season app tweak: soft "save anyway?" when every sensor field is blank (lab-3 observation).
4. ~~Activate v3 before lab 3~~ **DONE 2026-08-07; LAB 3 RAN ~08-10: SUCCESS — ALL groups' data arrived** (upload primary + a redesigned pre-lab session: in-class first submission with the sheet projected live; 2 groups used the email/AirDrop backup; 1 junk row hand-deleted; no in-WeChat uploads observed, path stays untested — full outcome row in `docs/instructor_overview.md`). **Season closed out 2026-08-12:** `UPLOAD_URL` blanked (endpoint archive + sheet deletion are manual steps on the project account); per-event-deployment lifecycle documented in SETUP_UPLOAD.md "Closing up" — secured by lifetime, not secrecy. Freeze over. Original activation notes: Endpoint deployed on a **dedicated project Google account**; `UPLOAD_URL` live (site stamp verified). Full rehearsal passed: phone → sheet (11 rows), edit + re-send → overwrote in place, real Google CSV download → Home Base on the Mac AND on the lab-day Windows laptop (rebuilt fresh from repo ZIP; old folder kept as `classroom_map_lab2_backup`). Mid-rehearsal find: iOS **Safari Websites location toggle off** → the app's "permanently denied" message — now a briefing/triage line. **Deliberately untested: sending from inside real WeChat** — lab 3 is the field test (overlay steering + email backup bound the risk). Dan updated the student/TA documents for the new flow. New docs: `docs/instructor_overview.md` (re-orientation for paper-writing) + `docs/lab_day_claude_context.md` (self-contained ops manual to paste into a claude.ai chat on lab day — no repo access needed). Pre-class reset: clear the sheet's `DW Test` rehearsal rows (health URL should report rows:0). After lab 3: record outcomes in the overview's timeline + article draft. Remaining v2.0 dream: live/Dash Home Base with auto-refreshing checklist; LAN/hotspot mode (same upload code path); institutional (non-Google) receiver as a drop-in.
