...
1. **Commit & push the v1.1 send/save update** (plus the two pending `classroom_map/` tweaks: scrollZoom, pillow in requirements) — the push deploys the web app via Actions. Then run the device matrix on the deployed URL: iPhone Safari + home-screen PWA, Android Chrome (share sheet expected); desktop Firefox (labelled saved-instead fallback expected). (`instructorEmail` is set to Dan's UTSC address.)
