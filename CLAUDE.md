...
- ✅ `flutter analyze` clean, 45 tests passing (added multi-file merge tests)
- ✅ Tested on physical iPhone (per user); ⏳ icon change not yet re-verified on device after rebuild

## Next Steps (priority order)
1. **Hub dry run with real data** — export a few phones' CSVs into `tool/classroom_map/csvs/`, run the launcher, project it, rehearse the "here's group N → all groups" debrief
2. **Re-verify icon on iPhone** — rebuild/reinstall (delete app first due to iOS icon caching); confirm home-screen icon now matches the in-app cloud
3. **Phase 2 — local-LAN sync** — small Flask server on the instructor's laptop/hotspot; add a "Sync to class" push in the app (cross-platform, no cloud) so the projected map fills as groups return
4. **Android build** — test APK once iOS is stable (switch to release signing first, see README)
